...
<section class="messages-section" aria-labelledby="messages-title">
    <div class="messages-heading">
        <div>
            ...
            <p>Your conversations with donors and food banks</p>
        </div>
    </div>

    <div class="message-search">
        <i class="fas fa-search"></i>
        <input type="search" id="message-contact-search" placeholder="Search individuals or food banks" autocomplete="off">
    </div>
    ...
    <section class="chat-panel" role="dialog" aria-modal="true" aria-labelledby="chat-contact-name">
        <header class="chat-header">
            <div class="chat-contact">
                <span id="chat-avatar-slot"><span class="message-avatar message-avatar--green">FB</span></span>
                <div><h2 id="chat-contact-name">Conversation</h2><p id="chat-contact-role">Messages</p></div>
            </div>
            <div class="chat-header-actions">
                <button class="chat-profile-btn" type="button" id="chat-profile-btn"><i class="far fa-user-circle"></i><span>View Profile</span></button>
                <button class="chat-close-btn" type="button" id="chat-close-btn" aria-label="Close chat"><i class="fas fa-xmark"></i></button>
            </div>
        </header>
...
(function () {
    ...
    function avatarMarkup(contact) {
        ...
        if (contact.avatar_url) return `<img class="message-avatar" src="${escapeHtml(contact.avatar_url)}" alt="${escapeHtml(contact.name)}">`;
    ...
    function renderConversation(conversation) {
        ...
        return `<button class="message-preview" type="button" data-contact-id="${contact.account_id}" role="listitem">${avatarMarkup(contact)}<span class="message-copy"><strong>${escapeHtml(contact.name)}</strong><span>${escapeHtml(preview)}</span></span><span class="message-time">${escapeHtml(last.time_label || '')}</span></button>`;
    }

    function renderContactResult(contact) {
        return `<button class="message-result" type="button" data-contact-id="${contact.account_id}">${avatarMarkup(contact)}<span><strong>${escapeHtml(contact.name)}</strong><small>${escapeHtml(contact.subtitle)}${contact.email ? ' - ' + escapeHtml(contact.email) : ''}</small></span></button>`;
    }

    function renderProfile(contact) {
        return `<div class="profile-card-header">${avatarMarkup(contact)}<div><h3>${escapeHtml(contact.name)}</h3><p>${escapeHtml(contact.subtitle)}</p></div></div><dl class="profile-details"><div><dt>ID</dt><dd>${escapeHtml(contact.custom_id || 'Not set')}</dd></div><div><dt>Email</dt><dd>${escapeHtml(contact.email || 'Not set')}</dd></div><div><dt>Phone</dt><dd>${escapeHtml(contact.phone || 'Not set')}</dd></div><div><dt>Address</dt><dd>${escapeHtml(contact.address || 'Not set')}</dd></div></dl>`;
    }
    ...
    function loadConversations() {
        fetch(`${apiBase}/get_conversations.php`).then(r => r.json()).then(data => {
            ...
            messagesEmpty.hidden = data.conversations.length !== 0;
        }).catch(error => {
    ...
    function searchContacts(query) {
        ...
        fetch(`${apiBase}/search_contacts.php?q=${encodeURIComponent(query.trim())}`).then(r => r.json()).then(data => {
            if (!data.success) throw new Error(data.message || 'Unable to search contacts');
            searchResults.hidden = false;
            searchResults.innerHTML = data.contacts.length ? data.contacts.map(renderContactResult).join('') : '<div class="message-result message-result--empty">No contacts found.</div>';
        }).catch(error => {
            searchResults.hidden = false;
    ...
    function openChat(contactId) {
        fetch(`${apiBase}/get_messages.php?contact_id=${encodeURIComponent(contactId)}`).then(r => r.json()).then(data => {
    ...
    messagesList.addEventListener('click', event => {
        ...
        if (preview) openChat(preview.dataset.contactId);
    });
    closeBtn.addEventListener('click', closeChat);
    profileBtn.addEventListener('click', () => {
        if (activeContact) profileCard.hidden = !profileCard.hidden;
    });
